...
CREATE TABLE `INBODY` (
  `ID` int(10) NOT NULL AUTO_INCREMENT COMMENT '編號 (PK)',
  `WEIGHT` decimal(5,2) NOT NULL COMMENT '體重 (kg)',
  `MUSCLE_MASS` decimal(5,2) NOT NULL COMMENT '肌肉量 (kg)',
  `TBW` decimal(5,2) NOT NULL COMMENT '身體總水量 (kg)',
  `PROTEINS` decimal(5,2) NOT NULL COMMENT '蛋白質重 (kg)',
  `MINERALS` decimal(5,2) NOT NULL COMMENT '礦物質重 (kg)',
  `BONE_CONTENT` decimal(5,2) NOT NULL COMMENT '骨質含量 (T)',
  `PBF` decimal(5,2) NOT NULL COMMENT '體脂肪率 (%)',
  `VFL` decimal(5,2) NOT NULL COMMENT '內臟脂肪 (%)',
  `BMR` decimal(7,2) NOT NULL COMMENT '基礎代謝率 (kcal)',
  `CREATOR` varchar(50) NOT NULL DEFAULT 'ADMIN' COMMENT '建立者',
  `CREATED_DATETIME` datetime NOT NULL COMMENT '建立日期時間',
  `MODIFIER` varchar(50) DEFAULT 'ADMIN' COMMENT '最後修改者',
  `MODIFIED_DATETIME` datetime DEFAULT NULL COMMENT '最後修改日期時間',
...
) ENGINE=InnoDB AUTO_INCREMENT=3 DEFAULT CHARSET=utf8 COMMENT='關聯表 - 會員 <-> INBODY';
